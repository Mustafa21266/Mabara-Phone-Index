...
exports.createExtension = async (req, res, next) => {
    ...
    if (!user || user.role !== 'admin') {
        ...
        let extension = await Extension.create(req.body)
...
exports.editExtension = async (req, res, next) => {
    const user = await User.findById(req.user._id)
    if (!user || user.role !== 'admin') {
        res.status(401).json({
            success: false,
        ...
        let extension = await Extension.findByIdAndUpdate(req.params.id, newData, {
            new: true,
            runValidators: true,
            useFindAndModify: false
        })
        extension = await Extension.findById(req.params.id)
        res.status(200).json({
                    success: true,
                    message: 'Extension Editied Successfully!',
                    extension
                })
        }
...
//             {
//                 folder: `dadswebextension/articles/images`
//             },
