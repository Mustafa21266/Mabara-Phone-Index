...
export const getUserDetails = (token) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.get(`http://localhost:8000/api/v1/getUserDetails/${token}`,config)
        dispatch({
...
export const loginAdmin = (loginData) => async (dispatch) => {
    ...
    try {
        ...
        const { data } = await axios.post(`http://localhost:8000/api/v1/login`, loginData, config)
        dispatch({
            type: LOGIN_ADMIN,
            payload: data
        })
...
export const forgotPassword = (forgotPasswordData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.post(`http://localhost:8000/api/v1/password/forgot`,  forgotPasswordData,config)
        dispatch({
...
export const resetPassword = (token, resetPasswordsData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8000/api/v1/password/reset/${token}`,  resetPasswordsData,config)
        dispatch({
            type: RESET_PASSWORD,
            payload: data
        })
...
export const editUserDetails = (id, userData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8000/api/v1/me/update/${id}`, userData, config)
        dispatch({
            type: EDIT_PROFILE,
            payload: data
        })
...
export const editUserDetailsAdmin = (id, userData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8000/api/v1/admin/user/update/${id}`, userData, config)
        dispatch({
...
export const deleteUserAdmin = (id) => async (dispatch) => {
    try {
        const { data } = await axios.delete(`http://localhost:8000/api/v1/admin/user/delete/${id}`)
        dispatch({
