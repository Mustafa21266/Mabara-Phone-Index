...
export const editFloor = (id,floorData) => async (dispatch) => {
    try {
        ...
        const { data } = await axios.put(`http://localhost:8000/api/v1/admin/floor/update/${id}`, floorData, config)
        dispatch({
            type: UPDATE_FLOOR,
            payload: data
        })
...
export const deleteFloor = (id) => async (dispatch) => {
    try {
        const { data } = await axios.delete(`http://localhost:8000/api/v1/admin/floor/delete/${id}`)
        dispatch({
            type: DELETE_FLOOR,
            payload: data
        })
...
export const getAllFloors = () => async (dispatch) => {
    try {
        const { data } = await axios.get(`http://localhost:8000/api/v1/floors/all`)
        dispatch({
            type: GET_ALL_FLOORS,
            payload: data
        })
