...
import { editDepartment, deleteDepartment } from "../../actions/departmentActions";
...
class EditDepartment extends Component {
  ...
  constructor(props) {
    super(props);
    this.state = {
      departmentEditied: false,
      nameArabic: store
        .getState()
        .department.departments.filter((department) => department._id === this.props.match.params.id)[0]
        .nameArabic,
        nameEnglish: store
        .getState()
        .department.departments.filter((department) => department._id === this.props.match.params.id)[0]
  ...
  async onSubmitHandler(e) {
    ...
    store
        .dispatch(editDepartment(this.props.match.params.id, formData))
        .then((data) => {
        if (data.success === true) {
            document.getElementById("loader").style.display = "none";
            toast.success(data.message);
            this.setState((state, props) => {
            return { departmentEditied: true };
            });
        } else {
            document.getElementById("loader").style.display = "none";
    ...
    store.dispatch(deleteDepartment(this.props.match.params.id)).then((data) => {
      if (data.success === true) {
        toast.success(data.message);
        this.setState((state, props) => {
          return { departmentEditied: true };
          ...
          <Fragment>
                            <div className="container">
                              <div className="row animate__animated animate__fadeIn animate__slower">
                                {this.state.departmentEditied ? <Redirect to="/" /> : ""}
